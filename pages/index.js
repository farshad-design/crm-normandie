// pages/index.js — Page principale CRM Immo Normandie
import { useState, useEffect } from 'react';
import dynamic from 'next/dynamic';
import Head from 'next/head';

// Leaflet ne supporte pas le SSR — chargement dynamique
...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/vendeurs')
      .then(r => r.json())
      .then(data => { setVendeurs(data); setLoading(false); })
      .catch(() => setLoading(false));
  }, []);

  const refreshVendeurs = () => {
    fetch('/api/vendeurs').then(r => r.json()).then(setVendeurs);
  };

  return (
    <>
      <Head>
        <title>CRM Immo Normandie — Caen 14000</title>
      ...
      <div style={{ display: 'flex', height: '100vh', fontFamily: 'system-ui, sans-serif' }}>
        ...
        <aside style={{
          ...
          <div style={{ padding: '18px 16px 14px', borderBottom: '1px solid #e5e7eb' }}>
            ...
            <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>Caen — Zone 14000+</div>
          </div>
          ...
          <div style={{ padding: 12, borderTop: '1px solid #e5e7eb', fontSize: 11, color: '#6b7280' }}>
            ...
            <div style={{ marginTop: 4 }}>{vendeurs.length} vendeurs chargés</div>
          </div>
        </aside>
        ...
        <main style={{ flex: 1, overflow: 'auto', background: '#f9fafb' }}>
          ...
          <header style={{
            ...
            <button
              onClick={() => window.open('/api/vendeurs', '_blank')}
              style={{
          ...
          <div style={{ padding: 20 }}>
            {tab === 'dashboard' && (
              <DashboardStats vendeurs={vendeurs} loading={loading} />
            )}
            {tab === 'carte' && (
              <MapCRM vendeurs={vendeurs} onVendeurSelect={(v) => console.log(v)} />
            )}
            {tab === 'vendeurs' && (
              <VendeursTable
                vendeurs={vendeurs}
                loading={loading}
                onRefresh={refreshVendeurs}
              />
            )}
            {tab === 'pipeline' && (
              <PipelineKanban vendeurs={vendeurs} onUpdate={refreshVendeurs} />
            )}
            {tab === 'agents' && (
              <AgentsPanel vendeurs={vendeurs} />
            )}
          </div>
        </main>
      </div>
